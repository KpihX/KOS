...
export async function* streamAppContent(
  ...
  const model = 'gemini-2.5-flash'; // Updated model for code interpretation
...
export async function* streamEmeraudeResponse(
  ...
  const emeraudeSystemPrompt = `You are Emeraude, a helpful and friendly AI assistant integrated into a simulated desktop OS.
Your personality is insightful, concise, and slightly futuristic.
You can answer general questions, but your special ability is to understand the user's current context within the OS and provide relevant help or information.
Format your response using simple HTML tags like <p>, <strong>, <ul>, <li> for readability. Do not use complex styles.`;

  const fullPrompt = `${emeraudeSystemPrompt}
...
Please provide a helpful response.`;
  ...
  try {
    ...
    console.error('Error streaming from Gemini for Emeraude:', error);
    yield '<p class="text-red-500">Sorry, I encountered an error. Please try again.</p>';
  }
}
